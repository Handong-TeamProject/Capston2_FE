...
function ProjectDetailPage() {
    ...
    return (
        <div className="w-full px-6 md:px-0">
                    ...
                    <div className="flex w-full flex-col mb-4">
                        <div>
                            <div className="flex justify-between items-start">
                                ...
                                <div>
                                    {isEdit &&
                                        <button className='text-black hover:text-lightGray text-sm mr-2' onClick={() => openCancleModal()}>취소하기</button>
                                    }
                                    <button className={`${isEdit ? 'text-orange hover:text-orange50' : 'text-lightGray hover:text-black'}  text-sm`} onClick={handleClickUpdateButton}>{isEdit ? "저장하기" : "수정하기"}</button>
                                </div>
                            </div>
                                    ...
                                    <div className="w-1/4 md:w-1/2 flex mb-3" key = {index}>
                                        <div className="pt-4">
                                            <div className="text-center">
                                                <img src = {`/Img/member${index+1}.png`} alt={member.name} className="w-12 h-12 md:w-16 md:h-16 rounded-full" />
                                                <p>{member.name}</p>
                                            </div>
                                        </div>
                                        { isEdit &&
                                            <div>
                                                <img src="/Img/cancleBefore.png"
                                                    onMouseEnter={(e) => e.currentTarget.src = "/Img/cancleAfter.png"}
                                                    onMouseLeave={(e) => e.currentTarget.src = "/Img/cancleBefore.png"}
                                                    alt="cancel" className="ml-1 md:ml-3 w-6 h-6 cursor-pointer" onClick={() => handleClickDeleteMember(member.user_id)} />
                                            </div>
                                        }
            ...
            {isUpdateModalOpen && (
                <ConfirmModal message = "정말 수정하시겠습니까?" closeModal={closeUpdateModal} handleAction={handleUpdateProject}/> 
            )}
            {isDeletedModalOpen && (
                <ConfirmModal message ="정말 삭제하시겠습니까?" closeModal={closeDeleteModal} handleAction={() => handleDeleteMember(deleteMember)}/> 
            )}
            {isCancleModalOpen && (
                <ConfirmModal message ="정말 취소하시겠습니까?" closeModal={closeCancleModal} handleAction={() => handleUpdateCancle()}/> 
            )}
        </div>
    );
}
