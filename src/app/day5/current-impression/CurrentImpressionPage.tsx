...
import { currentImpressionGetData } from '@/data/day5/currentImpressionGetData';
import { impressionQuestionList } from '@/data/common/impressionQuestionList';
import Image from "next/image";

interface QuestionWithAnswerList {
  question_id: number;
  first_answer_list?: { user_id: number; answer: number }[];
  now_answer_list?: { user_id: number; answer: number }[];
...
function FirstImpressionPage() {
  ...
  const [isWrite, setIsWrite] = useState(currentImpressionGetData?.is_done);
  const [isAllView, setIsAllView] = useState(true);
  const questionListIndex = currentImpressionGetData?.question_list || [];
  ...
  const questionPerDataFirst = calculateVotes(impressionData[currentQuestionIndex]?.first_answer_list || [], userList.map(user => user.user_id));
  const questionPerDataCurrent = calculateVotes(impressionData[currentQuestionIndex]?.now_answer_list  || [], userList.map(user => user.user_id));
          ...
          <div className="w-full">
            ...
            isAllView ? (
              <div className="w-full flex items-center mb-10 flex-col">
                {
                    assignedQuestionList.map((data, index) => (
                      <div className="w-full flex bg-beige90 flex-col sm:flex-row items-center mb-4 rounded-2xl" key={index}>
                        <div className="w-full flex flex-col items-center pt-5 max-w-[600px] mb-5 ">
                          {
                              ...
                              userList.map((data, index2) => (
                                <div key={index2} className="flex flex-col mx-2 h-full justify-end">
                                  <div
                                    ...
                                    style={{ height: `${calculateVotes(impressionData[index].first_answer_list || [], userList.map(user => user.user_id))[index2].votes * 30 + 30}px` }}
                                  >{calculateVotes(impressionData[index].first_answer_list || [], userList.map(user => user.user_id))[index2].votes}</div>
                                  <p>{data.name}</p>
                                </div>
                              ))
                            }
                        ...
                        <div className="w-full flex flex-col items-center max-w-[600px] mb-5 sm:mt-4">
                          {
                            <p className="text-xs lg:text-sm text-white bg-orange px-3 py-1 rounded-md mb-4">Q{index + 1}</p>
                          }
                          {
                            <p className="text-lg lg:text-xl mb-10 ">{assignedQuestionList[index].current_question}</p>
                          }
                          <div className="flex min-h-[180px] items-end">
                            {
                              userList.map((data, index2) => (
                                <div key={index2} className="flex flex-col mx-2 h-full justify-end">
                                  <div
                                    className={`${indexColor(index2)} flex items-end justify-center text-white rounded-t-lg`}
                                    style={{ height: `${calculateVotes(impressionData[index].now_answer_list || [], userList.map(user => user.user_id))[index2].votes * 30 + 30}px` }}
                                  >{calculateVotes(impressionData[index].now_answer_list || [], userList.map(user => user.user_id))[index2].votes}</div>
                                  <p>{data.name}</p>
                                </div>
                              ))
                            }
              ...
              <div className="w-full flex justify-center mb-10">
                <div className="w-full bg-beige90 rounded-2xl flex flex-col items-center py-5">
                  <div className="w-full flex flex-col sm:flex-row items-center ">
                    <div className="sm:w-1/2 flex flex-col items-center">
                      ...
                      <div className="flex min-h-[180px] items-end">
                        {
                          userList.map((data: { user_id: number; name: string }, index: number) => (
                          <div key={index} className="flex flex-col mx-2 h-full justify-end">
                            <div
                                ...
                                style={{ height: `${questionPerDataFirst[index].votes * 30 + 30}px` }}
                              >{questionPerDataFirst[index].votes}</div>
                              <p>{data.name}</p>
                            </div>
                          ))
                        }
                    ...
                    <div className="w-full sm:w-1/2 flex flex-col items-center mt-5 sm:mt-0 ">
                      {
                        <p className="text-xs lg:text-sm text-white bg-orange px-3 py-1 rounded-md mb-4">Q{currentQuestionIndex + 1}</p>
                      }
                      {
                        <p className="text-lg lg:text-xl mb-10 ">{assignedQuestionList[currentQuestionIndex].current_question}</p>
                      }
                      <div className="flex min-h-[180px] items-end">
                        {
                          userList.map((data: { user_id: number; name: string }, index: number) => (
                          <div key={index} className="flex flex-col mx-2 h-full justify-end">
                            <div
                                className={`${indexColor(index)} flex items-end justify-center text-white rounded-t-lg`}
                                style={{ height: `${questionPerDataCurrent[index].votes * 30 + 30}px` }}
                              >{questionPerDataCurrent[index].votes}</div>
                              <p>{data.name}</p>
                            </div>
                          ))
                        }
